...
atividade_1(char *host, int op)
{
	...
	cadastro cad[5];
   	...
   	if(operacao==2){
		...
		for(x=0; x<5; x++)
		{
			//strcpy(aux[x].usuario,cad[x].user);
			//strcpy(aux[x].msg,cad[x].mens);
			printf("Usuario: %s\n", cad[x].user);
			printf("Mensagem: %s\n", cad[x].mens);
	...
	if(*result_1 == 0){
	//Opcao cadastrar mensagem SUCESSO
	...
	if(*result_1 >= 100){
	//Quantidade mensagens apagadas (Opcao 3)			
		printf("Quantidade de mensagens apagadas: %d\n\n",*result_1-100);

		if(*result_1 - 100 == 0){
	
			printf("Nenhuma mensagem foi apagada.");
...
main (int argc, char *argv[])
{
	...
	while(1==1){
		...
		system("clear"); 
   		...
   		printf("<<< ATIVIDADE 1 - SISTEMAS DISTRIBUIDOS >>>\n\n");  
